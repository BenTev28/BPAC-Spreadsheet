...
Public Sub scheduleInit()
    ...
    Set sched = Worksheets("Schedule").ListObjects("eventItems")
    scheduleList.Clear
...
Public Sub itemInit()
    ...
    With tbl
        ...
        If Not .DataBodyRange.Rows.Count = 1 Then
            Set rngBlanks = Intersect(.DataBodyRange, .ListColumns("Venue").Range).SpecialCells(xlCellTypeBlanks)
            On Error GoTo 0
...
Private Sub boNotes_Change()
    ...
    Set ovwData = Worksheets("OverviewSheet").ListObjects("OverviewData")
...
Private Sub editItem_Click()
    ...
    With itemSelect
        ...
        Select Case .Column(0, .ListIndex)
            Case "ERH"
                Set useTable = [erh_Items]
            Case "NT"
                Set useTable = [nt_Items]
            Case "MH"
                Set useTable = [mh_Items]
            Case "BW"
                Set useTable = [bw_Items]
            Case "RS"
                Set useTable = [rs_Items]
            Case "B2"
                Set useTable = [b2_Items]
            Case "B3"
                Set useTable = [b3_Items]
            Case "SR"
                Set useTable = [sr_Items]
        End Select
    ...
    End With
    Set editRow = tbl.ListRows(index + 1)
...
Private Sub overviewCreate_Click()
    ...
    Set ventbl = Worksheets("Venues").ListObjects("Venues")
    Set data = ventbl.DataBodyRange
    firstDate = Format("1/1/9999", "dd/mm/yyyy")
    lastDate = Format("1/1/1900", "dd/mm/yyyy")
    
    Dim itemTbl As Range
    Set itemTbl = [costItems]
...
Private Sub scheduleAdd_Click()
    ...
    Set newRow = sched.ListRows.Add(AlwaysInsert:=True)
    
    newRow.Range = Array(scheduleDate.Value, scheduleStart.Value, scheduleEnd.Value, schedName.Value)
    With sched
        On Error Resume Next
        If Not .DataBodyRange.Rows.Count = 1 Then
            Set rngBlanks = Intersect(.DataBodyRange, .ListColumns("Date").Range).SpecialCells(xlCellTypeBlanks)
            On Error GoTo 0
            If Not rngBlanks Is Nothing Then
                rngBlanks.Delete
            End If
...
Private Sub scheduleCreate_Click()
    ...
    Set schedAll = Worksheets("Schedule").ListObjects("eventItems")
    Set schedRng = schedAll.DataBodyRange
